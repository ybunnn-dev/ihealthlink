...
<div>
    <div class="grid grid-cols-1 lg2:grid-cols-1 xl:grid-cols-5 gap-4 mb-3">
            ...
            <div class="bg-f7 rounded-xl p-6 text-center">
                <div class="flex items-center justify-center xl:justify-start gap-8 xl:gap-8 pl-4">
                    ...
                    <div class="flex flex-col">
                        <h1 class="text-sub_blue text-lg slg3:text-xl xl:text-xl 2xl:text-2xl font-bold mt-2">{{ $residents }}</h1>
    ...
    <div class="grid grid-cols-1 lg2:grid-cols-2 gap-3 mb-4">
    ...
    <div class="bg-white rounded-xl p-6 px-10 shadow-sm h-64 mb-3">
        ...
        <x-slot:thead>
            <tr>
                ...
                @foreach ($familiesPerPurok as $purok => $count)
         ...
         <x-slot:tbody>
            ...
            <tr class="border-t">
                ...
                @foreach ($familiesIndigentPerPurok as $purok => $count)
            ...
            <tr class="border-t">
                ...
                @foreach ($familiesPerPurok as $purok => $total)
                    @php
                        $indigent = $familiesIndigentPerPurok[$purok] ?? 0;
    ...
    <x-tables.purok-breakdown
        ...
        :headers="array_merge(['Category'], array_keys($householdsPerPurok), ['Total'])"
        :rows="[
            ['label' => 'Residents', 'values' => array_merge(array_values($residentsPerPurok), [array_sum($residentsPerPurok)])],
            ['label' => 'Households', 'values' => array_merge(array_values($householdsPerPurok), [array_sum($householdsPerPurok)])],
            ['label' => 'Families', 'values' => array_merge(array_values($familiesPerPurok), [array_sum($familiesPerPurok)])],
            ['label' => '4Ps-Members', 'values' => array_merge(array_values($families4PsPerPurok), [array_sum($families4PsPerPurok)])],
        ]"
    />
    ...
    <x-chart-layouts.donut-table-combo 
        ...
        <x-slot:thead>
            <tr>
                ...
                {{-- CHANGE: Loop directly over the keys of $malesPerPurok for the headers --}}
                @foreach (array_keys($malesPerPurok) as $purokName)
                    <th class="px-6 py-3 font-medium text-main_font border-b border-r text-center">{{ $purokName }}</th>
                @endforeach
    ...
    <x-chart-layouts.donut-table-combo 
        ...
        @php
            // Extract purok names
            $puroks = array_keys($malePwdsPerPurok);

            // Compute totals
            $maleTotal = array_sum($malePwdsPerPurok);
            $femaleTotal = array_sum($femalePwdsPerPurok);

            $purokTotals = [];
            foreach ($puroks as $purok) {
                $purokTotals[$purok] = ($malePwdsPerPurok[$purok] ?? 0) + ($femalePwdsPerPurok[$purok] ?? 0);
            }
        ...
        <x-slot:tbody>
            ...
            <tr class="border-t">
                <td class="px-6 py-3 font-medium border-r">Male</td>
                @foreach ($puroks as $purok)
                    <td class="px-6 py-3 text-center border-r">{{ $malePwdsPerPurok[$purok] ?? 0 }}</td>
                @endforeach
            ...
            <tr class="border-t">
                <td class="px-6 py-3 font-medium border-r">Female</td>
                @foreach ($puroks as $purok)
                    <td class="px-6 py-3 text-center border-r">{{ $femalePwdsPerPurok[$purok] ?? 0 }}</td>
                @endforeach
