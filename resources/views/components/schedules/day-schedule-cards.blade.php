@php
    // Color map remains available
    $dayColorMap = [
        'monday'    => '#279EFF', 'tuesday'   => '#328E6E', 'wednesday' => '#FFA673',
        'thursday'  => '#854836', 'friday'    => '#252F6C', 'saturday'  => '#EA5B6F',
        'sunday'    => '#799EFF',
    ];
    $defaultColor = '#697A8D';
@endphp
<div id="dailyContent" class="hidden grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 slg2:grid-cols-4 col-span-full gap-5">
    @foreach ($dailyActivities as $activity)
        <div class="bg-white rounded-xl p-6 flex flex-col items-center text-center space-y-4">
            ...
            {{-- 1. Conditionally display the correct SVG icon --}}
            @if ($activity->activities == 'No Activity')
                {{-- Use the default gray SVG when there is no activity --}}
                <svg class="w-12 h-12" viewBox="0 0 24 24" fill="none" xmlns="http://www.w.org/2000/svg">
                    <g>
            ...
            @else
                {{-- Otherwise, get the dynamic icon and color from the database --}}
                @php
                    $activityColor = $dayColorMap[strtolower($activity->day)] ?? $defaultColor;
                @endphp
                <svg class="w-12 h-12" style="color: {{ $activityColor }}" viewBox="{{ $activity->icon->viewbox }}"  fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                    {!! $activity->icon->path ?? '' !!}
                </svg>
            @endif
            ...
            <div>
                ...
                {{-- 2. Conditionally color the day's text --}}
                @if ($activity->activities == 'No Activity')
            ...
            {{-- 3. The button is the same in all cases, so it's now a single element --}}
            <button 
                id="manage-button-{{ strtolower($activity->day) }}"
                data-activity-id="{{ $activity->id }}" 
                data-activity-name="{{ $activity->activities }}" 
                data-activity-day="{{ $activity->day }}"
                data-activity-icon-id="{{ $activity->icon_id }}"
