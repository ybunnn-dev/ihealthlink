@props(['programfields', 'name', 'isTcl' => false])
...
<div class="col-span-1">
    <div class="flex justify-between items-center mt-4 mb-4">
        ...
        <div class="w-full sm:w-40">
             <button type="button" 
                     id="page-add-field-button" 
                     class="w-full h-[2.375rem] font-medium rounded-lg text-sm px-3
                            {{ $isTcl 
                               ? 'text-gray-400 bg-gray-200 cursor-not-allowed' 
                               : 'text-f7 bg-mainblue hover:text-mainblue hover:bg-nav_active' }}"
                     @if($isTcl) disabled @endif>
                Add Field
             </button>
        </div>
    </div>
                        ...
                        <tr class="bg-white border-b text-normal_font hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex justify-start items-start space-x-4">
                                    <button type="button" 
                                            class="edit-schedule-btn {{ $isTcl ? 'text-gray-400 cursor-not-allowed' : 'text-mainblue hover:text-blue-900' }}" 
                                            data-schedule-id="{{ $field->id }}" 
                                            title="{{ $isTcl ? 'Editing disabled for TCL programs' : 'Edit Field' }}"
                                            @if($isTcl) disabled @endif>
                                    ...
                                    <button type="button" 
                                            class="delete-schedule-btn {{ $isTcl ? 'text-gray-400 cursor-not-allowed' : 'text-red-600 hover:text-red-900' }}" 
                                            data-schedule-id="{{ $field->id }}" 
                                            title="{{ $isTcl ? 'Deletion disabled for TCL programs' : 'Delete Field' }}"
                                            @if($isTcl) disabled @endif>
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 pointer-events-none" viewBox="0 0 20 20" fill="currentColor">
                                          <path fill-rule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 011-1h4a1 1 0 110 2H8a1 1 0 01-1-1zm3 4a1 1 0 10-2 0v2a1 1 0 102 0v-2z" clip-rule="evenodd" />
                                        </svg>
                                    </button>
