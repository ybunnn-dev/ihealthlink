    ...
    let searchQuery = '', sortBy = 'name', dateFilter = 'all';
    ...
    const fetchBarangays = async (page = 1) => {
        ...
        try {
            ...
            // The backend now sends a JSON object with a 'data' array
    ...
    /**
     * Renders the table rows from the raw data array.
     */
    const renderTable = (barangays) => {
        ...
        barangays.forEach(barangay => {
            ...
            // Important: Slugify the name for the URL on the frontend
    ...
    /**
     * Renders pagination links from the 'links' array in the API response.
     */
    const renderPagination = (links) => {
        paginationContainer.innerHTML = '';
        if (!links || links.length <= 3) return; // Don't render if only prev/next/current exist
