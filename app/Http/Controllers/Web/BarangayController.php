...
class BarangayController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($filter === 'alpha_asc') {
            ...
            $query->orderBy('puroks_count', 'desc');
        }
    ...
    public function show(Barangay $barangay)
    {
        ...
        $barangay->puroks->each(function ($purok) {
            ...
            $purok->families_count = Family::whereHas('household', function ($query) use ($purok) {
                $query->where('purok_id', $purok->id);
            })
            ->where('status', 'active')
            ->count();

            $purok->residents_count = Resident::whereHas('family.household', function ($query) use ($purok) {
                $query->where('purok_id', $purok->id);
            })
            ->where('status', 'active')
            ->count();
